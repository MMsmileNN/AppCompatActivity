...
public class SwipeActivity extends AppCompatActivity {
    ...
    class SwipeLayout extends FrameLayout {
                ...
                switch (ev.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if ((dy == 0f || Math.abs(dx / dy) > 1) && (dx * dx + dy * dy > touchSlop * touchSlop)) {
                            ...
                            lastX = downX;
                            canSwipe = true;
                            tracker = VelocityTracker.obtain();//获得滑动速度测量对象
                            return true;
                        }
                        break;
                }
        ...
        public boolean onTouchEvent(@NonNull MotionEvent event) {
            ...
            if (canSwipe) {
                tracker.addMovement(event);//将事件加入到VelocityTracker类实例中
                ...
                switch (action) {
                    ...
                    case MotionEvent.ACTION_CANCEL:
                        ...
                        if (Math.abs(tracker.getXVelocity()) > mv) {
                            animateFromVelocity(tracker.getXVelocity());//获得当前x轴上的速度
                        } else {
